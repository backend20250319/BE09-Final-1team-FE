...
export function middleware(request: NextRequest) {
  ...
  if (isProtectedPath) {
    // ✅ HttpOnly 쿠키의 존재 여부만 확인합니다. 이름은 'access_token'으로 통일합니다.
    const accessTokenCookie = request.cookies.get("access_token")?.value;

    if (!accessTokenCookie) {
      console.log(`❌ [Middleware] '${pathname}' 접근에 인증 쿠키가 없어 로그인 페이지로 리디렉션합니다.`);
    ...
    console.log(`✅ [Middleware] '${pathname}' 접근에 인증 쿠키가 확인되어 접근을 허용합니다.`);
  }
