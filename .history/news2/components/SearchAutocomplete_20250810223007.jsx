...
export default function SearchAutocomplete({ 
  ...
  const fetchSuggestions = async (searchQuery) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/news/search?query=${encodeURIComponent(searchQuery)}&page=0&size=5`)
      if (response.ok) {
    ...
    <div className={`relative ${className}`} ref={dropdownRef}>
      <div className="relative">
        ...
        <Button
          ...
          className="absolute right-1 top-1/2 transform -translate-y-1/2 h-8 w-8 p-0 bg-blue-600 hover:bg-blue-700 text-white rounded-md"
        >
