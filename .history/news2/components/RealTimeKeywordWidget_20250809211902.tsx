...
export default function RealTimeKeywordWidget() {
  ...
  return (
    <div 
      ref={widgetRef}
      className={`border border-gray-300 bg-[#f8fafc] text-gray-900 hover:border-[#ff6b6b] hover:shadow-lg transition duration-200 rounded-xl p-4 overflow-visible glass hover-lift scroll-reveal ${isVisible ? 'revealed' : ''}`}
    >
      ...
      <div className="p-6 pt-0 overflow-visible">
        <div className="space-y-2">
          {keywords.map((item, index) => (
            <div 
              key={item.keyword} 
              className={`flex items-center justify-between p-2 rounded-lg hover:bg-white/50 transition-all duration-300 trending-keyword scroll-reveal scroll-reveal-delay-${index + 1} ${isVisible ? 'revealed' : ''}`}
            >
