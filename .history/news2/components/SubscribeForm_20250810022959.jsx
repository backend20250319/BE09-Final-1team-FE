    ...
    <div className="space-y-3">
      <form onSubmit={onSubmit} className={compact ? "flex gap-2" : "space-y-4"}>
        <Input
          ...
          className={`${compact ? 'h-10 text-sm' : 'h-12 text-base'} w-full px-4 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200 bg-white/90 backdrop-blur-sm`}
          disabled={loading}
        />
        <Button 
          type="submit" 
          disabled={loading} 
          className={`${compact ? 'h-10 text-sm' : 'h-12 text-base'} w-full font-semibold bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-lg shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed`}
        >
          {loading ? (
