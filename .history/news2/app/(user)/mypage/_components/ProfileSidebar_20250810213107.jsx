...
import { authenticatedFetch, getAccessToken, isAuthenticated } from "@/lib/auth"; // 인증된 요청을 위한 헬퍼 함수
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        console.log("ProfileSidebar API URL:", apiUrl);
        console.log("ProfileSidebar 환경 변수 확인:", {
          NEXT_PUBLIC_API_URL: process.env.NEXT_PUBLIC_API_URL,
          NODE_ENV: process.env.NODE_ENV
        });
        console.log("ProfileSidebar 인증 상태 확인:", {
        ...
        if (response && typeof response === 'object' && !response.ok && response.success === false) {
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="text-center text-red-500">
            <p className="font-medium mb-2">오류: {error}</p>
