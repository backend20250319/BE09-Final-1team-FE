...
import { dummyCategories, dummyUserData, createDummyResponse } from "@/lib/dummyData";
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        // authenticatedFetch가 에러 객체를 반환한 경우
        if (userResponse && typeof userResponse === 'object' && !userResponse.ok && userResponse.success === false) {
          console.warn("API 연결 실패, 더미 데이터 사용:", userResponse.message);
          setSelectedInterests(dummyUserData.data.hobbies || []);
          setNewsletterEnabled(dummyUserData.data.letterOk || false);
          return;
        }
        
        if (!userResponse || !userResponse.ok) {
          console.warn("API 응답 오류, 더미 데이터 사용");
          setSelectedInterests(dummyUserData.data.hobbies || []);
          setNewsletterEnabled(dummyUserData.data.letterOk || false);
          return;
        }
        ...
        if (userData.success) {
          ...
          console.warn("API 데이터 오류, 더미 데이터 사용:", userData.message);
          setSelectedInterests(dummyUserData.data.hobbies || []);
          setNewsletterEnabled(dummyUserData.data.letterOk || false);
        }
              } catch (err) {
          console.error("데이터 로드 오류:", err);
          console.warn("API 요청 실패, 더미 데이터 사용");
          setInterests(dummyCategories.data);
          setSelectedInterests(dummyUserData.data.hobbies || []);
          setNewsletterEnabled(dummyUserData.data.letterOk || false);
          setError("백엔드 서버 연결에 실패하여 더미 데이터를 사용합니다.");
        } finally {
          setIsLoading(false);
        }
    };
        ...
        <CardContent>
          ...
          {!isLoading && error && (
            <Alert className="mb-4">
              ...
              <AlertDescription>
                백엔드 서버 연결에 실패하여 더미 데이터를 사용하고 있습니다.
              </AlertDescription>
            </Alert>
          )}
