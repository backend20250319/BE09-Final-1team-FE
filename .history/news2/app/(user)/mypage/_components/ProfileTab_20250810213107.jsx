...
import { authenticatedFetch, getAccessToken, isAuthenticated } from "@/lib/auth";
      ...
      try {
        ...
        try {
          ...
          if (!interestsResponse.ok) {
            console.error("관심사 목록 응답:", interestsResponse.status, interestsResponse.statusText);
            throw new Error("관심사 목록 로딩 실패");
          }
          const interestsData = await interestsResponse.json();
          setInterests(interestsData.data);
        } catch (error) {
          console.error("관심사 목록 로딩 실패:", error);
          // 관심사 목록 로딩 실패 시에도 계속 진행
          setInterests([]);
        }
        ...
        if (userResponse && typeof userResponse === 'object' && !userResponse.ok && userResponse.success === false) {
          throw new Error(userResponse.message || "사용자 정보를 불러올 수 없습니다.");
        }
        
        if (!userResponse || !userResponse.ok) {
          console.error("사용자 정보 응답:", userResponse?.status, userResponse?.statusText);
