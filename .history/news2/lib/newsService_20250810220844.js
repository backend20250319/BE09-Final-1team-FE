...
class NewsService {
  ...
  async getAllNews(options = {}) {
    ...
    try {
      ...
      console.log('✅ 내부 API Route 성공:', data.content?.length || 0, '개')
      
      this.setCachedData(cacheKey, data)
      return data
    } catch (error) {
      console.error('❌ 내부 API Route 실패, 더미 데이터 사용:', error)
  ...
  async getNewsByCategory(category, options = {}) {
    ...
    try {
      ...
      const response = await fetch(`/api/news?page=${page}&size=${size}${categoryParam}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const data = await response.json()
      console.log('✅ 카테고리별 내부 API Route 성공:', category, data.content?.length || 0, '개')
      
      this.setCachedData(cacheKey, data)
      return data
    } catch (error) {
      console.error('❌ 카테고리별 내부 API Route 실패, 더미 데이터 사용:', error)
