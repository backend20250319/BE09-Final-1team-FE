...
export const newsletterService = {
    ...
    try {
      ...
      if (result.fallback) {
        ...
        try {
          ...
          console.log('💾 로컬 구독 상태 저장:', localSubscriptions);
        } catch (error) {
  ...
  async getEnhancedNewsletterData(options = {}) {
    try {
      ...
      // 백엔드 Enhanced API 직접 호출 시도
      const backendUrl = `${process.env.BACKEND_URL || 'http://localhost:8000'}/api/newsletter/enhanced?headlinesPerCategory=${headlinesPerCategory}&trendingKeywordsLimit=${trendingKeywordsLimit}${category ? `&category=${encodeURIComponent(category)}` : ''}`;
