...
export const newsletterService = {
  ...
  async unsubscribeNewsletter(category) {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}))
        console.error('❌ 구독 업데이트 실패:', { status: response.status, error: errorData })
        throw new Error(errorData.error || `HTTP error! status: ${response.status}`)
      }
  ...
  async getUserSubscriptions() {
    try {
      ...
      console.log('🔍 getUserSubscriptions 호출:', {
        url: `${baseUrl}/api/newsletters/user-subscriptions`,
        headers: headers
      })
      ...
      const mappedData = mapBackendToFrontend(data).mapSubscriptions(subscriptionsData)
