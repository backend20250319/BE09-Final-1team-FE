...
export function normalizeType(raw) {
    ...
    const m = {
        "정치":"POLITICS","경제":"ECONOMY","사회":"SOCIETY","생활":"LIFE","세계":"INTERNATIONAL",
        "IT/과학":"IT_SCIENCE","자동차/교통":"VEHICLE","여행/음식":"TRAVEL_FOOD","예술":"ART"
    };
...
class NewsService {
  constructor() {
    ...
    async summarizeById(newsId, { type, lines = 3, prompt, force } = {}) {
        ...
        return safeApiCall(api(`/api/news/${newsId}/summary`), {
            method: "POST",
            headers: { "Content-Type":"application/json" },
            body: JSON.stringify({ type: typeEnum, lines, prompt, force })
        });

        // authenticatedFetch가 fetch Response를 넘기는 경우
    ...
    async summarizeText(payload) {
        ...
        const url = api(`/api/news/summary`);
  ...
  async getNewsById(id) {
    ...
    try {
      ...
      // API 응답이 성공적이어도 데이터가 유효하지 않을 수 있습니다.
      if (!data) {
        console.error("API returned no data for news ID:", id);
        return null;
      }

      // `data` 객체 자체를 createNewsItem으로 전달합니다.
      const newsItem = createNewsItem(data);

      // newsItem이 유효한지 다시 한번 확인합니다.
      if (!newsItem) {
        console.error("Failed to create a valid news item from API response:", data);
        return null;
      }
