...
export async function POST(request) {
  try {
    ...
    if (
      body.hobbies &&
      (!Array.isArray(body.hobbies) || body.hobbies.length > 3)
    ) {
      return NextResponse.json(
        { success: false, message: "관심사는 최대 3개까지 선택 가능합니다." },
        { status: 400 }
      );
    }
    ...
    // 백엔드 API 호출
    const backendUrl = getBackendUrl("api/users/signup");
    ...
    const response = await fetch(backendUrl, {
      ...
      body: JSON.stringify({
        ...
        gender: body.gender,
        hobbies: body.hobbies || [], // 관심사 배열 추가 (없으면 빈 배열)
      }),
    });
