...
async function handler(request, { params }) {
  // 1. 요청 경로 조합
  // ex: /api/newsletter/mypage -> ['mypage'] -> "mypage"
  // ex: /api/newsletter/admin/1 -> ['admin', '1'] -> "admin/1"
  const path = params.slug ? params.slug.join('/') : '';

  // 2. 백엔드 API URL 생성
  // ex: http://localhost:8000/api/newsletter/22
  const backendUrl = `http://localhost:8085/api/${backendServicePath}/${path}`;
