...
export async function GET(request) {
  try {
    ...
    if (
      Array.isArray(headlinesData) &&
      headlinesData.length > 0 &&
      !headlinesData[0].views
    ) {
      console.log(
        "🔄 헤드라인에 조회수 정보가 없어 구독자 통계를 조회수로 설정"
      );
      ...
      try {
        ...
        const statsResponse = await fetch(
          `${
            process.env.BACKEND_URL || "http://localhost:8000"
          }/api/newsletter/stats/subscribers/category/${backendCategory}`,
          {
            method: "GET",
        ...
        if (statsResponse.ok) {
          ...
          const subscriberCount =
            statsData.data?.subscriberCount ||
            statsData.data?.activeSubscribers ||
            5000;
