...
export default function NewsPage() {
  ...
  useEffect(() => {
    const fetchRelatedNews = async () => {
      ...
      try {
        // newsService를 통해 관련 뉴스 조회 (BFF 패턴)
  ...
  useEffect(() => {
    const loadNewsData = async () => {
      ...
      try {
        const result = await newsService.getNewsById(articleId, true); // 읽기 기록 저장 활성화
        ...
        // newsService에서 이미 변환된 데이터를 받으므로 직접 사용
        const data = result;
