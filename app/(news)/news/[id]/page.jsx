...
const FontSizeSelector = ({ currentValue, onSelect, onClose }) => {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (selectorRef.current && !selectorRef.current.contains(e.target)) onClose();
    };
          ...
          <div className="relative flex flex-col items-center">
            <div className="relative flex w-full items-center justify-between">
              <div className="absolute left-0 top-1/2 w-full -translate-y-1/2">
                <div className="mx-auto h-0.5 w-[calc(100%-2rem)] bg-gray-200" />
              </div>
              {fontSizes.map((size) => {
                const isSelected = currentValue === size.value;
                return (
                    <button
                        key={size.id}
                        onClick={() => {
                ...
                <ul className="space-y-3">
                  {rankingNews.map((news, index) => (
                      <li key={news.id} className="flex items-center">
                        ...
                        <Link href={`/news/${news.id}`} className="hover:text-indigo-600 transition-colors flex-1">
                          {news.title}
                        </Link>
                      </li>
                  ))}
