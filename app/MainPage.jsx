...
export default function MainPage({
  ...
  const [loading, setLoading] = useState(false); // 초기 로딩 false (이미 화면 찬 상태)
  ...
  useEffect(() => {
    ...
    setLoading(false); // skeleton 끄기
  }, [listData?.content, listData?.totalPages, listData?.totalElements]); // 구체적인 필드만 의존성으로 사용
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10 max-w-[1200px] mx-auto">
              {listLoading && !initialList?.length ? (
                ...
                Array.from({ length: 20 }, (_, index) => (
                  <Card
                    key={`loading-${index}`}
                    className="min-h-[500px] max-h-[500px] w-full max-w-[600px] flex flex-col justify-between glass animate-pulse"
                  >
